...
export async function getOrCreateUserOrganization(userId: number) {
  ...
  // Create new organization for user - MySQL returns insertId
...
export async function createCustomer(data: any) {
  ...
  const insertId = Number(result.insertId);
  const inserted = await db.select().from(customers).where(eq(customers.id, insertId)).limit(1);
  return inserted[0];
}
...
export async function updateJob(id: number, data: any) {
  ...
  const { jobs } = await import("../drizzle/schema");
  const result = await db.update(jobs).set(data).where(eq(jobs.id, id));
...
export async function createJobStatus(data: any) {
  ...
  const { jobStatuses } = await import("../drizzle/schema");
  const result = await db.insert(jobStatuses).values(data);
  const insertId = Number(result.insertId);
  const inserted = await db.select().from(jobStatuses).where(eq(jobStatuses.id, insertId)).limit(1);
  return inserted[0];
}
...
export async function updateJobStatus(id: number, data: any) {
  ...
  const { jobStatuses } = await import("../drizzle/schema");
  const result = await db.update(jobStatuses).set(data).where(eq(jobStatuses.id, id));
  const updated = await db.select().from(jobStatuses).where(eq(jobStatuses.id, id)).limit(1);
  return updated[0];
}
...
export async function createJobStatusHistory(data: {
  ...
  const result = await db.insert(jobStatusHistory).values(data);
  const insertId = Number(result.insertId);
  const inserted = await db.select().from(jobStatusHistory).where(eq(jobStatusHistory.id, insertId)).limit(1);
  return inserted[0];
}
...
export async function createPersonnel(data: any) {
  ...
  const insertId = Number(result.insertId);
  const inserted = await db.select().from(personnel).where(eq(personnel.id, insertId)).limit(1);
  return inserted[0];
}
...
export async function updatePersonnel(id: number, data: any) {
  ...
  const { personnel } = await import("../drizzle/schema");
  const result = await db.update(personnel).set(data).where(eq(personnel.id, id));
...
export async function createConversation(data: any) {
  ...
  const insertId = Number(result.insertId);
  const inserted = await db.select().from(aiConversations).where(eq(aiConversations.id, insertId)).limit(1);
  return inserted[0];
}
...
export async function createMessage(data: any) {
  ...
  const insertId = Number(result.insertId);
  const inserted = await db.select().from(aiMessages).where(eq(aiMessages.id, insertId)).limit(1);
  return inserted[0];
}
...
export async function createMap(data: any) {
  ...
  const insertId = Number(result.insertId);
  const inserted = await db.select().from(maps).where(eq(maps.id, insertId)).limit(1);
  return inserted[0];
}
...
export async function createSite(data: any) {
  ...
  const insertId = Number(result.insertId);
  const inserted = await db.select().from(sites).where(eq(sites.id, insertId)).limit(1);
  return inserted[0];
}
...
export async function updateSite(id: number, data: any) {
  ...
  const { sites } = await import("../drizzle/schema");
  const result = await db.update(sites).set(data).where(eq(sites.id, id));
...
export async function createIntegrationConnection(data: any) {
  ...
  const { integrationConnections } = await import("../drizzle/schema");
  const result = await db.insert(integrationConnections).values(data);
  const insertId = Number(result.insertId);
  const inserted = await db.select().from(integrationConnections).where(eq(integrationConnections.id, insertId)).limit(1);
  return inserted[0];
}
...
export async function updateIntegrationConnection(id: number, data: any) {
  ...
  const { integrationConnections } = await import("../drizzle/schema");
  const result = await db.update(integrationConnections).set(data).where(eq(integrationConnections.id, id));
  const updated = await db.select().from(integrationConnections).where(eq(integrationConnections.id, id)).limit(1);
  return updated[0];
}
...
export async function createSyncLog(data: any) {
  ...
  const result = await db.insert(integrationSyncLogs).values(data);
  const insertId = Number(result.insertId);
  const inserted = await db.select().from(integrationSyncLogs).where(eq(integrationSyncLogs.id, insertId)).limit(1);
  return inserted[0];
}
...
export async function createMaintenanceTask(task: InsertMaintenanceTask) {
  ...
  const insertId = Number(result.insertId);
  const inserted = await db.select().from(maintenanceTasks).where(eq(maintenanceTasks.id, insertId)).limit(1);
  return inserted[0];
}
...
export async function createServicePlan(plan: InsertServicePlan) {
  ...
  const result = await db.insert(servicePlans).values(plan);
  const insertId = Number(result.insertId);
  const inserted = await db.select().from(servicePlans).where(eq(servicePlans.id, insertId)).limit(1);
  return inserted[0];
}
...
export async function createAuditLog(auditLog: InsertAuditLog) {
  ...
  const result = await db.insert(auditLogs).values(auditLog);
  const insertId = Number(result.insertId);
  const inserted = await db.select().from(auditLogs).where(eq(auditLogs.id, insertId)).limit(1);
  return inserted[0];
}
...
export async function bulkImportJobs(
  ...
  for (const jobData of jobsData) {
    try {
      ...
      const result = await db.insert(jobs).values(jobToInsert as any);
...
export async function createProduct(productData: any) {
  ...
  const result = await db.insert(productsComplete).values(insertData);
  const insertId = Number(result.insertId);
  const inserted = await db.select().from(productsComplete).where(eq(productsComplete.id, insertId)).limit(1);
  return inserted[0];
}
...
export async function createJobV2(data: InsertJob) {
  ...
  const insertId = Number(result.insertId);
  const inserted = await db.select().from(jobs).where(eq(jobs.id, insertId)).limit(1);
  return inserted[0];
}
...
export async function createMapFile(data: {
  ...
  const result = await db.insert(maps).values(insertData);
  const insertId = Number(result.insertId);
  const inserted = await db.select().from(maps).where(eq(maps.id, insertId)).limit(1);
  return inserted[0];
}
...
export async function createWaitlistEntry(data: InsertWaitlist) {
  ...
  const insertId = Number(result.insertId);
  const inserted = await db.select().from(waitlist).where(eq(waitlist.id, insertId)).limit(1);
  return inserted[0];
}
...
export async function createApiKey(data: any) {
  ...
  const insertId = Number(result.insertId);
  const inserted = await db.select().from(apiKeys).where(eq(apiKeys.id, insertId)).limit(1);
  return inserted[0];
}
...
export async function createJobShare(data: any) {
  ...
  const result = await db.insert(jobShares).values(data);
  const insertId = Number(result.insertId);
  const inserted = await db.select().from(jobShares).where(eq(jobShares.id, insertId)).limit(1);
  return inserted[0];
}
